...
#pipieline for fastp trimming, alignment and variant calling
##sonia cebrian camison scebrian27@gmail.com/sonia.cebrian@ebd.csic.es
###10/09/2024

### 1: fastp trimming (culex conda env on talapas)
...
### 2. alineamiento (culex conda env on talapas)
...
3. contro coverage along the genome (to-do) 
...
FALTA COVERAGE CHECK Y METER SCRIPT QUALIMAP
...
To do so I use <variant-filter1n.sh>.
...
```
#### 
These vcf do not include information on quality by depth (QD), which is necessary for the notebook so I add it using <qd.sh>: <qd.sh bash qd.sh pipiens123_1_filter1n.vcf.gz pipiens123_1_filter1n_qd.vcf.gz> for each chromosome.
#### qd.sh
```bash
#!/usr/bin/env bash
zcat $1 | gawk '
...
TODO 
* FILTERS: depth, mapping quality, minimum allele frequency
* filter linkage desequilibrium (PLINK)
#### detalles plink
De esto que pongo no se si los dos son codigos que porbo mari o si la segunda parte es el output ya 

```bash
# perform linkage pruning - i.e. identify prune sites, pero cambio la ventana segun papers de mosquitos QUITAR LINKED
plink --vcf  merged.vcf.gz --double-id --allow-extra-chr \
--set-missing-var-ids @:# \
--indep-pairwise 50 10 0.5 --out culex2

# Esto me permite quedarme con mas SNPS, en este caso, quita solo 4818243 de 11532685
# prune and create pca pero me vuelve a coger 11 millones y lo he cambiado y no me sale. EXTRAER LINKED Y HACER FILES PARA PCA

plink --vcf merged.vcf.gz --double-id --allow-extra-chr --set-missing-var-ids @:# 
--extract culex2.prune.in \
--make-bed --pca --out culex2
```
####
* ADMIXTURE /STRUCTURE analyses: en principio structure es mas robusto cuando no hay una clara estrcutra poblacional pero es muy sensible a unaven sampling por localidades asi que tener en cuenta. Structure (ESPAÑA)
...
* PCA (plink) vs Local PCA ¿cual hacer y por qué? 
  * local PCA se hace tambien con biallelic positions o con todo? ¿con todo por inversiones entiendo? o sea si no como se puede ver que sitios estan relacionados con inversiones y cuales no?
* landscape analysis. alternativa a circuit theory?

#### otras preguntas

* meter molestus en el PCA para ver como interactuan con los nbornal

# Other checks and analyses
### A. mtDNA to identify species
In this case, the workflow above was run using the complete reference genome, an not only the 3 main chromosomes. This is because the mitochondrial sequence was needed to identify the species

To cut the mitochondrial reads form the original merged bams I used the script <mito-array-bams.sh>. To run this script i use the same sample list I used to merge the bams (<list-to-merge-bams.txt>).

```bash
sbatch --array=1-3 -n 1 --cpus-per-task=1 --mem-per-cpu=2G -t 02:00:00 --job-name=3rep.index  mito-array-bams.sh 3.repetidas list-merged-bams.txt 
```
I then transfer the mtDNA files and refernece genome to geneious. 
Now I obtain the consensus sequence of mitogenome for each sample by doing "Tools" > "Generate Consensus Sequence". These are the options I used:

* Threshold: Highest quality (60%) for the consensus sequence.
* Assign Quality: Total quality assigned to the consensus sequence.
* Call "N" if coverage < 8: Positions with coverage below 8 are marked as "N."
* Trim to reference sequence: Ensures the consensus is the same length as the reference.
* Sanger Heterozygotes: Calls heterozygotes if >50% of reads support an alternative allele.

I obtained reference mithochondrion genome for Anopheles cruzzi form NCBI: https://www.ncbi.nlm.nih.gov/nuccore/MK575477.1?report=fasta 
Then i used it as outgroup to construct a Tree in Geneious prime
(si have to put the settings here) 

i think they were that
Alignment Type: "Global alignment with free end gaps" 
Cost Matrix (65% similarity)
Genetic Distance Model i recall using jukes cantor
Tree Build Method (Neighbor-Joining)
Gap Open Penalty (12) & Gap Extension Penalty (3)

thats all the info i have and there should be an audio on my phone
 maybe with more explanations

AQUI FALTA HACER LA PARTE EN LA QUE USO TB OTRAS ESPECIES DE CULEX Y LOS MOLESTUS DEL PAPER QUE ENCONTRÉ PARA COMPARAR
